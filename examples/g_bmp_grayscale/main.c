...
int main(int argc, char *argv[]) {
    ...
    if (image.Load(&image, "sample-01.bmp")) {
        ...
        if (image.selectColor(&image, &output, (g_rgb_t){254, 254, 183}, (g_hsi_t){0.8, 0.1, 0.5})) {
            output.Save(&output, "sample-01_yellow_color.bmp");
        }
    ...
    if (image.Load(&image, "sample-01.bmp")) {
        ...
        if (image.selectColorRange(&image, &output, (g_rgb_t){248, 255, 190}, (g_rgb_t){216, 139, 94})) {
            output.Save(&output, "sample-01_yellow_range.bmp");
        }
